...
class resnet18_with_position_attention(nn.Module):
    def __init__(self, block=BasicBlock, layers=[2, 2, 2, 2], num_classes=5, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
...
def resnet34_with_position_attention(pretrained=False, progress=True, **kwargs):
    return _resnet34_with_position_attention('resnet34_with_position_attention', BasicBlock, [3, 4, 6, 3], pretrained, progress,
                   **kwargs)


def _resnet34_with_position_attention(arch, block, layers, pretrained, progress, **kwargs):
    model = resnet34_with_position_attention(block, layers, **kwargs)
    return model


class resnet34_with_position_attention(nn.Module):
    def __init__(self, block=BasicBlock, layers=[3, 4, 6, 3], num_classes=5, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        PA = self.pam_attention(x,x,x)
        #draw_features(PA, 'resnet34_with_spatial_attention_Resblock1')
        x = self.layer2(PA)
        x = self.layer3(x)
        x = self.layer4(x)
        x = self.avgpool(x)
...
class plain_vote_feature_map_resnet18(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        #PA4 = self.pam_attention4(x)
        x1 = self.avgpool(x1)
        logits_fundus = self.cls_fundus(x1.view(x1.size(0),-1))
